...
- **Architecture pivot, not micro-tuning:** stop spending time on Kaliski thresholds/local Solinas. Strongest current route is now the **full BY ratio selector**: `full_ratio_state_streams_all_branches_in_560_bits` validates that `h0=x*p^-1 mod 2^560` plus closed ratio updates streams all 560 branch bits exactly and tapers to zero. The fixed 560-qubit register can hold remaining ratio plus consumed history, fitting the ~600-bit low-qubit selector allowance without denominator pair, x-column residues, carry rows, or separate tail history. `full_ratio_initial_constant_multiply_is_not_the_main_blocker` estimates the h0 constmul roundtrip at ~221k--443k Toffoli (summed add widths=110,720; p^-1 popcount=305), so initialization is probably okay. But `ratio_a_step_is_inverse_dense_and_common` shows the direct A-step is gate-dangerous: toy 16-bit A map high bit has ANF degree 14 and density 7268/32768; real traces average ~132 A-steps. Windowing helps only partly: W=16 v2(m01) hist is `[0,347,673,380,317,164,104,55,49,17,18,2,2,9,9,2,92]`, so 45.5% have v2 1-2; W=32 still has 822/1088 windows with v2<=4. Full-ratio is state-optimal, not yet gate-optimal; next work is a better inverse-maintenance circuit or fallback to linear carry. Older selector results remain fallback diagnostics. See `src/point_add/by_sota_architecture.md`.
