...
use super::{
    ...
    kaliski_iteration_bulk_prefix3_backward, with_kal_inv_raw, Op, QubitId, B, N, SECP256K1_P,
};
...
fn build_generic_prefix_fb(k: usize) -> PrefixCircuit {
    ...
    for i in (0..k).rev() {
        kaliski_iteration_backward(&mut b, SECP256K1_P, &u, &v, &r, &s, m_hist[i], f, i, None);
    }
    PrefixCircuit {
        ops: b.ops,
        num_qubits: b.next_qubit as usize,
    ...
    fn bulk_prefix3_step_matches_generic_on_reachable_states() {
        ...
        for sample_idx in 0..512usize {
            if (sample_idx & 63) == 0 {
                check_deadline(
                    deadline,
                    "kaliski_equiv::bulk_prefix3_step_matches_generic_on_reachable_states",
                );
            }
            let v0 = sampler.next();
            for iter_idx in 0..3 {
    ...
    fn bulk_prefix3_step_matches_generic_on_deep_reachable_states() {
        ...
        for sample_idx in 0..128usize {
            if (sample_idx & 31) == 0 {
                check_deadline(
                    deadline,
                    "kaliski_equiv::bulk_prefix3_step_matches_generic_on_deep_reachable_states",
                );
            }
            let v0 = sampler.next();
    ...
    fn bulk_prefix3_step_matches_classical_transition() {
        ...
        for sample_idx in 0..512usize {
            if (sample_idx & 63) == 0 {
                check_deadline(
                    deadline,
                    "kaliski_equiv::bulk_prefix3_step_matches_classical_transition",
                );
            }
            let v0 = sampler.next();
            for iter_idx in 0..3 {
    ...
    fn bulk_prefix3_three_step_sequence_matches_generic_forward_state() {
        ...
        for sample_idx in 0..512usize {
            if (sample_idx & 63) == 0 {
                check_deadline(
                    deadline,
                    "kaliski_equiv::bulk_prefix3_three_step_sequence_matches_generic_forward_state",
                );
            }
            let v0 = sampler.next();
    ...
    fn bulk_prefix3_backward_matches_generic_three_step_inverse() {
        ...
        for sample_idx in 0..256usize {
            if (sample_idx & 63) == 0 {
                check_deadline(
                    deadline,
                    "kaliski_equiv::bulk_prefix3_backward_matches_generic_three_step_inverse",
                );
            }
            let v0 = sampler.next();
            let g = run_step_circuit(
    ...
    fn bulk_prefix3_step_matches_generic_on_late_safe_prefix_states() {
        ...
        for sample_idx in 0..128usize {
            if (sample_idx & 31) == 0 {
                check_deadline(
                    deadline,
                    "kaliski_equiv::bulk_prefix3_step_matches_generic_on_late_safe_prefix_states",
                );
            }
            let v0 = sampler.next();
            for (j, &iter_idx) in iter_points.iter().enumerate() {
    ...
    fn with_kal_inv_raw_phase_equivalence_small_prefix() {
        ...
        for sample_idx in 0..64usize {
            if (sample_idx & 15) == 0 {
                check_deadline(
                    deadline,
                    "kaliski_equiv::with_kal_inv_raw_phase_equivalence_small_prefix",
                );
            }
            let v0 = sampler.next();
    ...
    fn prefix_phase_and_mhist_equivalence_grid() {
        ...
        for sample_idx in 0..48usize {
            if (sample_idx & 15) == 0 {
                check_deadline(
                    deadline,
                    "kaliski_equiv::prefix_phase_and_mhist_equivalence_grid",
                );
            }
            let v0 = sampler.next();
            for (j, &k) in ks.iter().enumerate() {
