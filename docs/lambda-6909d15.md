...
> whose `src/` tree is identical to accepted submission **`ed4b529`** — the five commits on top are
> CI-only. Sweep ran at **6 workers**, not 14.
>
...
official source."* That sentence was true on 2026-07-28 and is now stale. `cf5aa02` is **14 commits
behind `upstream/main`**, and eight submissions have been accepted since:
...
`cf5aa02` is also an ancestor of this repo's HEAD — we rebased past it on 2026-08-03
([`rebase-2026-08-03-upstream-ed4b529.md`](rebase-2026-08-03-upstream-ed4b529.md)). And it is
...
Identical to the `801dd20` sweep: 202 trials, each a full `./benchmark.sh` — build plus a
...
3. **Worker isolation.** `eval_circuit` writes `results.tsv` to the `CARGO_MANIFEST_DIR` baked in
   at compile time, so workers sharing a build would append to one file. Verified empirically —
   a trial in `w01` grew `w01/results.tsv` and left `w00`'s untouched — not merely by rebuilding.
...
|---|---|---|---|---|---|
| classical | **17.126** | 4.149 | ±0.294 | 1.005 | 7–29 |
| phase | **11.628** | 3.473 | ±0.246 | 1.037 | 3–22 |
| ancilla | 0 | 0 | — | — | 0 |

var/mean ≈ 1 on both channels — Poisson, no overdispersion, same as on `801dd20`. Ancilla garbage
is identically zero because `B::free` emits an unconditional `R`, so the channel cannot fire.
...
independent components, so `Cov(c,p) = λ_both` and `λ_total = mean_c + mean_p − Cov`.
...
CI is a 4,000-resample bootstrap over whole rows (seed 20260804). The `sum(means)` sem is the
independent-sum `sqrt(sem_c² + sem_p²)`, the same convention `lambda-measurement.md` uses.

The directional caveat from `lambda-measurement.md` carries over unchanged: the estimator assumes
the classical-only and phase-only components are mutually independent. If they are positively
correlated beyond the shared term, λ_both is overstated and **λ_total is overestimated**, moving
down toward the 17.13 lower bound. The error is one-way, so the covariance estimate remains
...
Both *per-channel* means did rise, each marginally: classical +0.894 ± 0.400 (t = 2.24, p = 0.025),
phase +0.714 ± 0.336 (t = 2.12, p = 0.034). λ_total absorbs most of it because λ_both rose in step
(7.111 → 8.193), which is why the channel-level movement does not propagate to the total. Given two
uncorrected comparisons and p-values near 0.03, this is weak evidence of a real per-channel rise —
...
`02-lambda.md`'s 23.29 remains the stalest figure of the three, and is the one this measurement is
offered to replace.
...
Welch t = +0.770 (classical) and −0.089 (phase) — indistinguishable, reproducing the `801dd20`
isolation result on a different head with a different base nonce. The closest any trial came to
clean was `base+29` at 8 classical / 4 phase. Grinding near a known-good nonce buys nothing.
...
`801dd20`. Losing 8 workers cost only 11% of throughput — consistent with that document's finding
that the harness contends on memory bandwidth and I/O (each trial pushes ~507 MB through zstd),
not on arithmetic. Per-worker that is ~118 s/trial under 6-way contention; a single uncontended
`./benchmark.sh` on a cold machine measured 84.6 s here.

**6 workers is the better operating point on this hardware** — near-identical throughput, and it
...
that the CI spans a factor of ~150 in wall-time — λ enters exponentially, so every figure here is
an order of magnitude, not a quantity.
...
([`../tools/nonce-screen/`](../tools/nonce-screen/)) buys ≈ 2.2 of them. λ reduction itself remains
the gating project.
...
The analysis code was validated by re-deriving every published `801dd20` figure from
