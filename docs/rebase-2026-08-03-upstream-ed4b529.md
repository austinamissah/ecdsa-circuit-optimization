...
*below* 1,486,468,554. It cannot, and the bound does not depend on how good my miner is:
...
- My miner is a strict conservative subset. On the old head it realized 0.897% where a full strip
...
## The identity gate: my two source changes survive the rebase exactly

46 commits replayed onto `upstream/main` with **zero conflicts**, since upstream touched none of the
three files I carry (`trailmix_ludicrous/gcd.rs`, `trailmix_ludicrous/schedule.rs`,
...
- **A new, deeper strip table**: **13,056 dead / 4,222 downgrade, 0 stale**, against my
...
monotonic appends with zero retractions are backwards for census *deepening*, and I inferred "an
...
**So: upstream does not budget phase risk explicitly, and the `3.0` is not my λ_phase_only.** My
...
the same shape as the λ-versus-strip trade I measured. That is a statement about their process,
...
1. **Editable surface is enforced twice.** `benchmark.json` sets
   ...
   `git diff --name-only $SHA^ $SHA` must be under `src/point_add/`. My `docs/` and `tools/`
...
3. **The 45-minute cap is not currently a threat.** My full build+eval is ~110 s on a throttling
