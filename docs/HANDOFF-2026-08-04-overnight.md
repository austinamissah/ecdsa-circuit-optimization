...
(1,487,599,474) both quote out-of-date figures. See
...
shots**, and one such gate would clear the 0.886 bar. Not harvestable: that is a certificate about
...
from source into
`/tmp/claude-1000/.../scratchpad/solvers/{kissat,cadical}/build/`, so rebuild them before
...
instance**, since the 25 inputs are pinned to literal bit patterns, so no break is emitted for it,
...
- **Baseline portfolio: running, no result.** 14 diversified arms (kissat
  ...
  ~21:57, no wall cap. `RESULT` absent, `status.tsv` empty, so every arm still searching, which
...
- **Symmetry-break selftest: did not return in the window.** It builds exact-9 (known SAT via
  ...
  and no UNSAT from it may be believed**, because a break that loses a solution turns an open question
  into a false UNSAT.
...
`/tmp` is not durable. Everything that matters is committed; the dumps and solver binaries are
