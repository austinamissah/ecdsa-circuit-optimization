...
//!     That is what `tools/census/census.rs` certifies, and it is expensive to
//!     establish and fragile under stream edits.
...
//!     opidx  kind  c2  c1  t  cond_bit  charge  hotness
//! where `charge` is shots-charged out of `n_shots` and `hotness = charge/n_shots`.
